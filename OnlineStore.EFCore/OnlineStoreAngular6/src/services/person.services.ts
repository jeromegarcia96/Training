...
export class PersonService {
    ...
    getPersonWithPagination(page: number, itemsPerPage: number, filterVal: string) {
        return this.http.get("https://localhost:44351/api/Person/"+ page + "/" + itemsPerPage + "?filter=" + filterVal)
        .toPromise()
    ...
    addPerson(objEntity: Person) {
        return this.http.post("https://localhost:44351/api/Person", objEntity)
        .toPromise()
        .then(data => {return data as Person });
    }

    editPerson(id, objEntity: Person) {
        return this.http.put("https://localhost:44351/api/Person/" + id, objEntity)
        .toPromise()
        .then(data => {return data as Person });
    }
